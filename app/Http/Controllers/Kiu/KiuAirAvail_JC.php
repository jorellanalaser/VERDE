    ...
    public function index()
    {
        ...
        else
        {
          ...
          //dd($rs);
            ...
            //  JCCV Cambio realizado por Julio Colmenares, controla compras desed USA - MIAMI
            if( ($userisocountry['isoCode']) == 'US')
            {
                if ((($rs->request->Segments[0]->Origin) == 'CCS' and ($rs->request->Segments[0]->Destination) == 'MIA') or
                    (($rs->request->Segments[0]->Origin) == 'MIA' and ($rs->request->Segments[0]->Destination) == 'CCS'))
                    return view('RutaNoDisponible');
                    //include ("RutaNoDisponible.html");
            }
            // Fin cambio de JCCV
